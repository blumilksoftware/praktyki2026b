...
const onKeyDownEnter = () => {
  if (isOpen.value && filteredOptions.value.length > 0) {
    ...
    triggerError(t('dynamicList.errors.invalidSelection', { fallback: 'Wybierz element z listy podpowiedzi' }))
  }
}
...
<template>
  <div class="relative flex flex-col gap-1 w-full">
    <label :for="props.id" class="sr-only">
      {{ $t('dynamicList.accessibility.label', { fallback: 'Wyszukaj i wybierz elementy z listy' }) }}
    </label>
    ...
    >
      <TransitionGroup
        ...
        leave-active-class="transition duration-200 ease-in absolute" leave-from-class="opacity-100 scale-100"
      ...
      >
        ...
        >
          ...
          <button
            ...
            :aria-label="$t('dynamicList.accessibility.removeItem', { item, fallback: `Usuń element ${item}` })"
      ...
      <button
        ...
        :aria-label="isOpen ? $t('dynamicList.accessibility.closeMenu', { fallback: 'Zamknij menu' }) : $t('dynamicList.accessibility.openMenu', { fallback: 'Otwórz menu' })"
    ...
    >
      <div
        v-if="isOpen"
        ref="dropdownRef"
        class="absolute z-50 left-0 right-0 top-[52px] border rounded-md shadow-lg bg-background border-border max-h-60 overflow-y-auto"
      >
        <ul 
          :id="`${props.id}-listbox`"
          role="listbox"
          :aria-label="$t('dynamicList.accessibility.suggestions', { fallback: 'Podpowiedzi' })"
          class="py-1"
        >
          <li
          ...
          <li 
            ...
            role="status"
            aria-live="polite"
            class="px-4 py-3 text-sm text-additional text-center"
          >
            <div v-if="searchQuery && allowCustom">
              {{ $t('dynamicList.pressEnterToCreate', { fallback: `Naciśnij Enter, aby dodać "${searchQuery}"` }) }}
            </div>
            <div v-else>
    ...
    <button
      ...
      :aria-label="$t('dynamicList.accessibility.clearAll', { fallback: 'Usuń wszystkie wybrane elementy' })"
