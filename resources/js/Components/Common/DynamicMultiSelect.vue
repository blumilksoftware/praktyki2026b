...
const inputPlaceholder = computed(() => {
  ...
  return props.placeholder || t('components.dynamicList.placeholder')
})
...
function selectOption(option) {
  ...
  if (props.max && props.modelValue.length >= props.max) {
    triggerError(t('components.dynamicList.errors.maxReached', { max: props.max }))
    return
  }

  if (!value) {
    triggerError(t('components.dynamicList.errors.empty'))
    return
  }

  if (props.modelValue.includes(value)) {
    triggerError(t('components.dynamicList.errors.exists'))
    return
  }

  if (!props.allowCustom && !normalizedOptions.value.some((item) => item.value === value)) {
    triggerError(t('components.dynamicList.errors.invalidSelection'))
    return
  }
    ...
    <div class="relative w-full">
      ...
      >
        <div class="flex min-w-0 flex-1 flex-wrap items-center gap-1.5">
          ...
          >
            ...
            <button
              ...
              :aria-label="t('components.dynamicList.accessibility.removeItem', { item: optionLabel(item) })"
          ...
          <input
            ...
            :aria-label="t('components.dynamicList.accessibility.inputLabel')"
        ...
        <button
          ...
          :aria-label="isOpen ? t('components.dynamicList.accessibility.closeMenu') : t('components.dynamicList.accessibility.openMenu')"
      ...
      >
        ...
        >
          <ul :id="`${props.id}-listbox`" role="listbox" :aria-label="t('components.dynamicList.accessibility.suggestions')" class="py-1">
            <li
            ...
            <li v-if="filteredOptions.length === 0" role="status" aria-live="polite" class="px-4 py-3 text-center text-additional text-sm">
              {{ searchQuery ? t('common.empty.noResults') : t('components.dynamicList.emptyState') }}
            </li>
          </ul>
        </div>
      </Transition>
