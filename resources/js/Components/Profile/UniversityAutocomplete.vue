...
import { useMapboxGeocoding } from '@/Composables/useUniversitySearch'
...
const { suggestions, fetchSuggestions, clearSuggestions } = useUniversityS()
...
const selectUniversity = (suggestion) => {
...
<template>
  <div class="relative">
    <BaseInput :id="id" :model-value="displayValue" :label="label" :placeholder="placeholder" :error="error" :required="required" :stacked="stacked"
    ...
    >
      <li v-for="(university, index) in suggestions" :key="university.name" class="cursor-pointer px-4 py-2 text-sm"
