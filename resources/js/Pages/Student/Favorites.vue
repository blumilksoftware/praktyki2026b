  ...
  <BaseLayout active-page="favorites" :nav-items="navItems" :logo-href="ROUTES.STUDENT_DASHBOARD" background-class="bg-white" :show-background="false" layout-scope="student" :show-user-section="false" :show-compact-menu="true" :compact-menu-items="navItems">
    <div class="min-h-screen bg-background px-4 py-6 sm:px-6 lg:px-8">
      <div class="mx-auto mb-4 flex max-w-7xl items-center justify-between gap-3">
        ...
        <div class="inline-flex items-center gap-2 rounded-full border border-border bg-background px-4 py-2 text-sm font-semibold text-text">
          {{ t('student.nav.favorites') }}
          <span class="rounded-full bg-white px-2 py-0.5 text-xs text-additional">{{ favoriteCount }}</span>
        </div>
      </div>
      ...
      <section class="mx-auto max-w-7xl rounded-3xl border border-border/80 bg-white/90 p-6 shadow-[0_14px_40px_rgba(11,26,48,0.08)] backdrop-blur-sm sm:p-8">
        <div class="flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between">
          ...
          <button
            type="button"
            class="inline-flex items-center justify-center rounded-xl border border-border bg-white px-5 py-3 text-sm font-semibold text-text transition hover:border-primary/40 hover:bg-background focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary/30 disabled:cursor-not-allowed disabled:opacity-50"
        ...
        <div class="mt-6">
          ...
          <div v-else class="rounded-3xl border border-dashed border-border/80 bg-white/75 p-8 text-center text-additional backdrop-blur-sm">
            ...
            <Link
              ...
              class="mt-5 inline-flex items-center justify-center rounded-xl border border-primary bg-primary px-5 py-3 text-sm font-semibold text-white transition hover:bg-secondary focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary/40"
            >
